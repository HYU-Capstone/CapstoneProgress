...
def background_task():
  while True:
    ...
    # print(files)
    if len(files) >= 5:
      classify_process(files, sess, graph, model, class_names)
...
def classify_process(imagepaths, sess, graph, model, class_names):
  with mutex:
    start = time.time()
    current_time = str(time.time())
    sess_dir = 'tmp/' + current_time
    os.mkdir(sess_dir)
    for filepath, index in zip(imagepaths, range(1, 6)):
      filename = str(index) + '.jpg'
      shutil.move(filepath, os.path.join(sess_dir, filename))
    align_result = align_dataset_mtcnn(current_time)
    if align_result == None:
      ...
      return
    print(align_result)
    tracking(align_result[0], align_result[1], current_time)
    try:
      with graph.as_default():
